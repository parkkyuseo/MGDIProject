...
def main():
    ...
    ap.add_argument('--mp-det-conf', type=float, default=None,
                    help='min_detection_confidence (0.0~1.0). If not set, falls back to --min-conf.')
    ap.add_argument('--mp-track-conf', type=float, default=None,
                    help='min_tracking_confidence (0.0~1.0). If not set, falls back to --min-conf.')
    ap.add_argument('--mp-model-complexity', type=int, choices=[0, 1], default=1,
                    help='Hands model complexity (0=light, 1=base).')
    ap.add_argument('--mp-max-hands', type=int, default=1,
                    help='Max number of hands to detect (keep 1 to avoid competition).')

    ap.add_argument('--rect-alpha', type=float, default=0.75,
                    help='stereoRectify free-scaling (0=crop/zoom-in, 1=keep FOV).')

    # ---- [GESTURE-ANGLE] CLI options ----
    ap.add_argument('--gesture', action='store_true',
                    help='Enable angle-based Open/Closed gesture (no model file needed)')
    ap.add_argument('--gesture-every', type=int, default=1,
                    help='Run angle-based gesture every Nth pair')
    ...
    inst = h3d.initialize(
        ...
        mp_det_conf=args.mp_det_conf,
        mp_track_conf=args.mp_track_conf,
        mp_model_complexity=args.mp_model_complexity,
        mp_max_hands=args.mp_max_hands,
        rect_alpha=args.rect_alpha,
        # angle-based gesture (on/off & interval)
        ...
        while True:
            ...
            if out is not None:
                ...
                if good:
                    ...
                    # META
                    if 'ts_src_ms' in out:       payload['ts_src_ms'] = int(out['ts_src_ms'])
                    if 'detected_now' in out:    payload['detected_now'] = bool(out['detected_now'])
                    if 'interpolated' in out:    payload['interpolated'] = bool(out['interpolated'])
                    # gesture (angle-based)
                    if 'gesture' in out and isinstance(out['gesture'], dict):
                        payload["gesture"] = {
                            "name": str(out['gesture'].get("name","")),
                            "score": float(out['gesture'].get("score", 0.0))
                        }

                    # === joints 변환/첨부 ===
                    if 'joints_c' in out and isinstance(out['joints_c'], (list, tuple)):
                        joints_out = []
                        for jc in out['joints_c']:
                            if (not jc) or (jc[0] is None):
                                joints_out.append([None, None, None, 0.0])
                                continue
                            Xc = np.array([jc[0], jc[1], jc[2]], dtype=np.float64)
                            Xc_u = apply_input_conv(Xc, input_conv_use)
                            if R_HC is not None:
                                Xh = R_HC @ Xc_u + t_HC
                                joints_out.append([float(f"{Xh[0]:.6f}"),
                                                   float(f"{Xh[1]:.6f}"),
                                                   float(f"{Xh[2]:.6f}"),
                                                   float(jc[3])])
                            else:
                                joints_out.append([float(f"{Xc_u[0]:.6f}"),
                                                   float(f"{Xc_u[1]:.6f}"),
                                                   float(f"{Xc_u[2]:.6f}"),
                                                   float(jc[3])])
                        payload["joints"] = joints_out  # index=0..20 (MediaPipe 순서)
            ...
            if (now - last_status_t) >= max(0.05, float(args.status_every)):
                ...
                try:
                    if out and 'gesture' in out and isinstance(out['gesture'], dict):
                        status['gesture'] = {
                            "name": str(out['gesture'].get("name","")),
                            "score": float(out['gesture'].get("score", 0.0))
                        }
