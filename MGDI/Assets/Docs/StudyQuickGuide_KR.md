...
4. 각 컨디션 시작 시 phone app을 실행 또는 재실행하고, phone으로 QR 코드를 스캔한 뒤 HoloLens의 `Phone QR detected.` 안내를 기다립니다.
5. 자세 안내와 예시 이미지가 나오면, 안내된 자세를 잡고 잠시 유지합니다.
6. 카운트다운이 끝나면 과제가 자동으로 시작됩니다. 별도의 `Start` 버튼은 없습니다.
7. 각 태스크 시작 전 짧은 `practice`가 나올 수 있습니다. `practice`는 기록되지 않습니다.
8. 목표를 맞춘 뒤 준비가 되면 `triple tap`으로 제출합니다.
9. 한 컨디션이 끝나면 다음 컨디션으로 자동 진행됩니다.
...
- 각 컨디션마다 phone으로 QR 코드를 스캔하고 `Phone QR detected.` 안내를 확인한 뒤 자세를 맞추고 `triple tap` 합니다.
